...
'''
==========================================
NxosSystem() - nxos_system.py
==========================================

Description
-----------
NxosSystem() generates Ansible task instances conformant with its identically-named Ansible module.
...
domain_lookup               When enabled, the system will try to resolve hostnames
                            When disabled, hostnames will not be resolved::
                                ...
                                - Type: str()
                                - Valid values: no, yes
domain_name                 Configures the default domain name suffix to be used 
                            when referencing this node by its FQDN::

                                - Type: str()
                                - Example: instance.domain_name = spine01.bar.com
domain_search               Configures a list of domain name suffixes to search
                            when performing DNS name resolution::

                                - Type: list()
                                - Example: instance.domain_search = ['foo.com', 'bar.com']
                                - TODO: figure out dict() keys.  domain and vrf?
hostname                    Configure the device hostname parameter::

                                - Type: str() or keyword 'default'
name_servers                List of DNS name server IP addresses to use to perform name
                            resolution lookups::

                                - Type: list() of str()
                                - Example: task.name_servers = ['1.2.3.4', '5.6.7.8']
                                - Example: task.name_servers = [{'server': '1.2.3.4', 'vrf': 'mgmt'}]
...
system_mtu                  System maximum transfer unit::
                                ...
                                - Type: int()
                                - Valid values: int() range: 1500-9216, or keyword 'default'
========================    ===========
'''
