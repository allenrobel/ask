...
'''
Name: nxos_igmp_snooping.py

Description:

NxosIgmpSnooping() generates Ansible Playbook tasks conformant with nxos_igmp_snooping
which can be fed to AnsPlaybook().add_task()

Example usage:
    unit_test/cisco/nxos/nxos_igmp_snooping.py

Properties:

    bool() valid values are: no, yes

    group_timeout           - str()     Group membership timeout value for all VLANs on the device. 
                                        Valid values: int() range 1-10080, never, default.
    link_local_grp_supp     - bool()   Global link-local groups suppression.
    report_supp             - bool()   Global IGMPv1/IGMPv2 Report Suppression
    snooping                - bool()   Enables/disables IGMP snooping on the switch
    state                   - str()     Valid values: default, present
    v3_report_supp          - bool()   Global IGMPv3 Report Suppression and Proxy Reporting
'''
